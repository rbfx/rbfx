...
| Élément | Rôle |
|---|---|
| `BlueprintDefs.h/.cpp` | Identifiants, types de pins, types de données de base et génériques, structs, enums, delegates, timelines, macros, nœuds, liens, variables, commentaires, fonctions et diagnostics. |
| `BlueprintGraph.h/.cpp` | Conteneur du graphe, CRUD, validation, sérialisation JSON versionnée, migration, recherche, placement automatique, commentaires, fonctions/sous-graphes, structs, enums, delegates, timelines et macros. |
| `BlueprintRuntime.h/.cpp` | Registre extensible, exécution déterministe, variables, fonctions paramétrées, événements d’entrée, Delay/Tick, collections, delegates/signaux, timelines, macros, nœuds 2D/3D et nœuds spécialisés de physique, animation, audio, caméra et matériaux. |
...
| `TestBlueprint.cpp` | Tests Catch2 du graphe, JSON et migrations, types avancés, collections, delegates/signaux, timelines, macros, édition logique, fonctions paramétrées, runtime latent, ressource, réflexion, registre et debugger. |

## Nœuds intégrés

Le registre fournit les nœuds de flux `Event.OnStart`, `Event.OnKeyPressed`, `Event.OnMouseClick`, `Flow.Branch`, `Flow.Print` et `Flow.Delay`, les nœuds mathématiques `Math.AddFloat`, `Math.MultiplyFloat`, `Math.LessFloat`, `Math.SubtractFloat`, `Math.DivideFloat`, `Math.ClampFloat`, `Math.LerpFloat`, `Math.SinFloat`, `Math.CosFloat`, `Math.AddVector3` et `Math.ScaleVector3`, les variables `Variable.Get` et `Variable.Set`, ainsi que `Function.Entry`, `Function.Return` et `Function.Call`. Les familles `Array.Make/Get/Set/Length/Add/Remove`, `Map.Make/Get/Set/Contains`, `Struct.Make/Get/Set` et `Enum.Value` manipulent les données avancées définies par le graphe. Les familles `Delegate.Bind/Unbind/Call/IsBound` et `Signal.Connect/Disconnect/Emit` fournissent les événements typés ; `Timeline.Play/Pause/Stop/GetValue` et `Macro.Call` couvrent les séquences et réutilisations de graphes. Les primitives de scène couvrent les positions, translations, rotations et échelles 2D/3D : `Scene.GetPosition2D/3D`, `SetPosition2D/3D`, `Translate2D/3D`, `GetRotation2D/3D`, `SetRotation2D/3D`, `GetScale2D/3D` et `SetScale2D/3D`. Les familles natives spécialisées comprennent `Physics`, `Animation`, `Audio`, `Camera` et `Materials`, avec prise en charge des composants rbfx 2D/3D et des références de ressources. Le registre est ouvert : un module ou un jeu peut enregistrer ses propres nœuds avec un nom stable, une catégorie, une description, un mode d’exécution, des pins et une fonction C++.
...
La validation réalisée sur cette branche a donné **230 tests réussis sur 230** dans la suite complète. Les tests Blueprint et rbscript couvrent le graphe, les commentaires, les fonctions paramétrées, les sous-graphes sérialisés, les types avancés, les collections, les delegates/signaux, les timelines, les macros, le runtime latent, les événements d’entrée, la migration de schéma, les ressources natives, la réflexion, le débogage, le lexer, le parseur, le système de types, le compilateur, le VM, les bindings et l’interopérabilité. La compilation de l’éditeur produit également `build-editor/bin/Debug/Editor` sous Linux.
...
La validation automatisée complète réalisée dans l’environnement disponible a été effectuée sous **Linux x86_64** : compilation de `Editor` et **230 tests réussis sur 230**. Windows et macOS restent couverts par la matrice GitHub Actions, qui exécute désormais les tests natifs sur les variantes macOS et un job dédié `rbscript-validation` sur Linux, Windows MSVC, macOS ARM64 et macOS x64 ; ces runners distants doivent fournir le verdict matériel propre à chaque plateforme.
...
Le cœur de l’expérience Blueprint et les extensions principales demandées sont maintenant disponibles et compilables : types structurés, collections, delegates/signaux, timelines, macros, nœuds spécialisés pour physique, animation, audio, caméra et matériaux, ainsi que les panneaux de types de l’éditeur. Les évolutions futures concernent surtout les interfaces Blueprint, le reroutage manuel des câbles, les watch expressions évaluables, le profiling par nœud et la création/édition encore plus riche des signatures de fonctions directement dans l’éditeur.

La documentation complémentaire du langage textuel, du bytecode, du VM, des bindings, des collections, du débogueur et de l’éditeur rbscript se trouve dans [`Documentation/RbScript.md`](RbScript.md). Les prochaines étapes de production pourront ajouter une factory dédiée au Resource Browser pour créer et renommer les fichiers `.blueprint` depuis l’interface, ainsi que la liaison automatique du Blueprint à l’objet ou au Node sélectionné dans la scène. Le format est désormais versionné en **schéma 5** et accepte les fichiers historiques de formats 1 à 4 ; les migrations sont appliquées dans `BlueprintGraph::FromJSON`, avec des collections vides lorsque les anciens graphes ne possèdent pas encore structs, enums, delegates, timelines ou macros.
