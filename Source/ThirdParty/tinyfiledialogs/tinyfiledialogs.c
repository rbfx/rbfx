...
const int tinyfd_zenity = 1;
const int tinyfd_zenity3 = 2;
const int tinyfd_matedialog = 3;
const int tinyfd_qarma = 4;
const int tinyfd_kdialog = 5;
const int tinyfd_osascript = 6;

int tinyfd_getDesktopDialog()
{
    // MacOS
    if (isDarwin() && osascriptPresent())
        return tinyfd_osascript;

    // If this is Qt desktop - try checking Qt utilities first.
    char* desktop = getenv("XDG_SESSION_DESKTOP");
    if (strcmp(desktop, "KDE") == 0 || strcmp(desktop, "lxqt") == 0)
    {
        // qt first
        if (kdialogPresent())
            return tinyfd_kdialog;
        if (qarmaPresent())
            return tinyfd_qarma;
    }

    // Otherwise most of other desktops are gtk-based so check gtk tools.
    if (zenity3Present() == 3)
        return tinyfd_zenity3;
    if (zenityPresent())
        return tinyfd_zenity;
    if (matedialogPresent())
        return tinyfd_matedialog;

    // If this was not Qt desktop and no gtk dialog utilities were found - try Qt ones.
    if (kdialogPresent())
        return tinyfd_kdialog;
    if (qarmaPresent())
        return tinyfd_qarma;
...
"-e 'if vButton is \"Yes\" then' -e 'return 1' -e 'else if vButton is \"No\" then' -e 'return 2' -e 'else' -e 'return 0' -e 'end if' " );
	...
	else if ( deskop_utility == tinyfd_zenity || deskop_utility == tinyfd_matedialog || deskop_utility == tinyfd_qarma )
	{
		if ( deskop_utility == tinyfd_zenity )
		{
			if (aTitle&&!strcmp(aTitle,"tinyfd_query")){strcpy(tinyfd_response,"zenity");return 1;}
			strcpy ( lDialogString , "szAnswer=$(zenity --" ) ;
		}
		else if ( deskop_utility == tinyfd_matedialog )
		{
...
");if [ $? = 1 ];then echo 0;elif [ $szAnswer = \"No\" ];then echo 2;else echo 1;fi");
	...
	else if ( deskop_utility == tinyfd_kdialog )
	{
...
"-e 'set mystring to mystring & \" \" & ((item i of mycolor) div 256 as integer) as string' " );
	...
	else if ( deskop_utility == tinyfd_zenity3 || deskop_utility == tinyfd_matedialog || deskop_utility == tinyfd_qarma )
	{
		lWasZenity3 = 1 ;
		if ( deskop_utility == tinyfd_zenity3 )
		{
			if (aTitle&&!strcmp(aTitle,"tinyfd_query")){strcpy(tinyfd_response,"zenity");return (char const *)1;}
			sprintf ( lDialogString ,
