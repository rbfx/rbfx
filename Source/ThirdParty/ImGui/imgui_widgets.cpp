...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_TRUNC((bb_frame_width - 2.0f * style.PointSize) * 0.5f), 0.0f, 3.0f * style.PointSize), -ImClamp(IM_TRUNC((bb_frame_height - 2.0f * style.PointSize) * 0.5f), 0.0f, 3.0f * style.PointSize)));
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ...
    const float border_size = (border_col.w > 0.0f) ? g.Style.ChildBorderSize : 0.0f;
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (active)
    {
        const float pad = ImMax(style.PointSizeRound, IM_TRUNC(square_sz / 6.0f));
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.StyleTemplate.FramePadding.x, g.StyleTemplate.WindowPadding.y)); // Horizontally align ourselves with the framed text
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    // Calculate bounds
    const float grab_padding = IM_TRUNC(2.0f * style.PointSize); // FIXME: Should be part of style.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            float bg_offy_dn = is_multiline ? 0.0f : IM_TRUNC(2.0f * style.PointSize);
