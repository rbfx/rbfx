...
inline std::shared_ptr<logger> systemd_logger_mt(const eastl::string &logger_name)
{
    return Factory::template create<sinks::systemd_sink_mt>(logger_name);
}

template<typename Factory = spdlog::synchronous_factory>
inline std::shared_ptr<logger> systemd_logger_st(const eastl::string &logger_name)
{
