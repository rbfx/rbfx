...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    else
    {
        ...
        float w = window->MenuColumns.DeclColumns(label_size.x, 0.0f, ImFloor(g.FontSize * 1.20f)); // Feedback to next frame
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        float w = window->MenuColumns.DeclColumns(label_size.x, shortcut_size.x, ImFloor(g.FontSize * 1.20f)); // Feedback for next frame
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
