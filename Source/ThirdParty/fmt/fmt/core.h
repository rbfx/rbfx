...
#define FMT_USE_CUSTOM_BASIC_STRING_ALLOCATOR EASTLAllocatorType
#endif
...
template <typename Char, typename Alloc=FMT_USE_CUSTOM_BASIC_STRING_ALLOCATOR> using std_basic_string = FMT_USE_CUSTOM_BASIC_STRING<Char, Alloc>;
...
/**
  An implementation of ``std_basic_string_view`` for pre-C++17. It provides a
  subset of the API. ``fmt::basic_string_view`` is used for format strings even
  if ``std_string_view`` is available to prevent issues when a library is
...
template <typename Char> class basic_string_view {
 ...
 public:
  ...
  /** Constructs a string reference from a ``std_basic_string`` object. */
  template <typename Alloc>
  FMT_CONSTEXPR basic_string_view(const std_basic_string<Char, Alloc>& s)
...
/**
  ...
  **Example**::
    ...
    std_string message = fmt::format(my_string("The answer is {}"), 42);
  \endrst
 */
...
inline basic_string_view<Char> to_string_view(
    const std_basic_string<Char/*, Traits*/, Allocator>& s) {
...
template <typename Char>
struct is_contiguous<std_basic_string<Char>> : std::true_type {};
...
template <typename Char>
std_basic_string<Char> vformat(basic_string_view<Char> format_str,
...
inline std_basic_string<Char> vformat(
...
/**
  ...
  **Example**::
    ...
    #include <fmt/core.h>
    std_string message = fmt::format("The answer is {}", 42);
  \endrst
*/
// Pass char_t as a default template parameter instead of using
// std_basic_string<char_t<S>> to reduce the symbol size.
template <typename S, typename... Args, typename Char = char_t<S>>
inline std_basic_string<Char> format(const S& format_str, Args&&... args) {
