...
void TessellationReferenceGL(ISwapChain* pSwapChain)
{
    ...
    glShaders[0] = pEnv->CompileGLShader(GLSL::TessTest_VS, GL_VERTEX_SHADER);
    ASSERT_NE(glShaders[0], 0u);
    glShaders[1] = pEnv->CompileGLShader(GLSL::TessTest_TCS, GL_TESS_CONTROL_SHADER);
    ASSERT_NE(glShaders[1], 0u);
    glShaders[2] = pEnv->CompileGLShader(GLSL::TessTest_TES, GL_TESS_EVALUATION_SHADER);
    ASSERT_NE(glShaders[2], 0u);
    glShaders[3] = pEnv->CompileGLShader(GLSL::TessTest_FS, GL_FRAGMENT_SHADER);
    ASSERT_NE(glShaders[3], 0u);
