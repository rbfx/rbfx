...
bool Model::BeginLoad(Deserializer& source)
{
    ...
    if (!BeginLoadImpl(source))
        return false;

    // Read metadata
    auto* cache = GetSubsystem<ResourceCache>();
    const ea::string xmlName = ReplaceExtension(GetName(), ".xml");
...
bool Model::BeginLoadImpl(Deserializer& source)
{
    auto& name = source.GetName();
    if (name.ends_with(".obj", false))
    {
        return LoadOBJ(source);
    }

    return LoadMDL(source);
}
