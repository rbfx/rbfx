...
static const ea::unordered_map<ea::string, VertexElementSemantic> sSemanticsMapping = {{"iPos", SEM_POSITION},
...
bool ShaderProcessor::ProcessHLSL()
{
    ...
    case CS:
        ...
        profile = "cs_5_0";
        break;
    default: URHO3D_LOGERROR("Unsupported ShaderType {}", desc_.type_); return false;
    }
    ...
    HRESULT hr = D3DPreprocess(sourceCode.data(), sourceCode.length(), desc_.name_.c_str(), &macros.front(), nullptr,
        &processedCode, &errorMsg);

    if (processedCode)
    {
        destroyableBlobs.push_back(processedCode);
        sourceCode = ea::string((const char*)processedCode->GetBufferPointer(),
            (const char*)processedCode->GetBufferPointer() + processedCode->GetBufferSize());
    }
    if (errorMsg)
    {
        destroyableBlobs.push_back(errorMsg);
        ea::string output((const char*)errorMsg->GetBufferPointer(),
    ...
    // We normalize buffers to: CameraVS => Camera, ObjectPS => Object, ZonePS => Zone
    {
        ...
        {
            sourceCode.replace(
                Format("{}{}", shaderParameterGroupNames[i], cbufferSuffix), shaderParameterGroupNames[i]);
        } while (shaderParameterGroupNames[++i] != nullptr);
    }

    // We must generate shader bytecode to execute reflection
    ID3DBlob* byteCode = nullptr;
    hr = D3DCompile(sourceCode.data(), sourceCode.length(), desc_.name_.c_str(), nullptr, nullptr,
        desc_.entryPoint_.c_str(), profile, flags, 0, &byteCode, &errorMsg);
    ...
    switch (desc_.type_)
    {
    case VS: cbufferSuffix = "VS"; break;
    case PS: cbufferSuffix = "PS"; break;
    case GS: cbufferSuffix = "GS"; break;
    case HS: cbufferSuffix = "HS"; break;
    case DS: cbufferSuffix = "DS"; break;
    case CS: cbufferSuffix = "CS"; break;
    default: URHO3D_LOGERROR("Unsupported ShaderType {}", desc_.type_); return false;
    }
    ...
    if (desc_.type_ == VS)
    {
        ...
        for (uint8_t i = 0; i < shaderDesc.InputParameters; ++i)
        {
            ...
            if (semantic == MAX_VERTEX_ELEMENT_SEMANTICS)
                continue;
            vertexElements_.push_back(VertexElement(GetElementType(paramDesc), semantic, paramDesc.SemanticIndex));
    ...
    for (unsigned i = 0; i < shaderDesc.ConstantBuffers; ++i)
    {
        ...
        for (unsigned j = 0; j < cbDesc.Variables; ++j)
        {
            ...
            if (nameStart != ea::string::npos)
            {
                ...
                parameters_[varName] =
                    ShaderParameter{desc_.type_, varName, varDesc.StartOffset, varDesc.Size, cbRegister};
            }
        }
    }
...
{
    if (!CompileGLSLToSpirV(desc_.type_, desc_.sourceCode_, desc_.macros_, byteCode, compilerOutput_))
        return false;
    if (desc_.optimizeCode_)
    {
        std::vector<unsigned> tmpByteCode = OptimizeSPIRV(std::vector<unsigned>(byteCode.begin(), byteCode.end()),
            SPV_ENV_OPENGL_4_0, SPIRV_OPTIMIZATION_FLAG_LEGALIZATION);
    ...
    ea::vector<ea::pair<unsigned, VertexElementSemantic>>& inputLayout)
{
    ...
    if (desc_.type_ == VS)
    {
        ...
        auto GetElementType = [](SpvReflectInterfaceVariable* variable)
        {
            ...
            case SPV_REFLECT_FORMAT_R32_SINT: type = TYPE_INT; break;
            case SPV_REFLECT_FORMAT_R32_SFLOAT: type = TYPE_FLOAT; break;
            case SPV_REFLECT_FORMAT_R32G32_UINT:
            case SPV_REFLECT_FORMAT_R32G32_SINT:
            case SPV_REFLECT_FORMAT_R32G32_SFLOAT: type = TYPE_VECTOR2; break;
            case SPV_REFLECT_FORMAT_R32G32B32_UINT:
            case SPV_REFLECT_FORMAT_R32G32B32_SINT:
            case SPV_REFLECT_FORMAT_R32G32B32_SFLOAT: type = TYPE_VECTOR3; break;
            case SPV_REFLECT_FORMAT_R32G32B32A32_SFLOAT: type = TYPE_VECTOR4; break;
            case SPV_REFLECT_FORMAT_R32G32B32A32_UINT:
            case SPV_REFLECT_FORMAT_R32G32B32A32_SINT:
            case SPV_REFLECT_FORMAT_R64_UINT:
            case SPV_REFLECT_FORMAT_R64_SINT: type = TYPE_UBYTE4; break;
            }
        ...
        for (SpvReflectInterfaceVariable** it = inputVars.begin(); it != inputVars.end(); ++it)
        {
            ...
            unsigned slotIdx = inputNameEndIdx == inputName.length() - 1
                ? 0
                : ToUInt(inputName.substr(inputNameWithoutIdx.length(), inputName.length() - 1));
            ...
            vertexElements_.push_back(VertexElement(GetElementType(*it), semantic, slotIdx));
    ...
    for (auto descBindingIt = descriptorBindings.begin(); descBindingIt != descriptorBindings.end(); descBindingIt++)
    {
        ...
        if (binding->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
        {
            ...
            {
                URHO3D_LOGERROR(
                    "Failed to reflect shader constant buffer for {} shader. Invalid constant buffer name: {}",
                    desc_.name_, bindingName);
            ...
            while (membersCount--)
            {
                ...
                if (nameStart != ea::string::npos)
                    varName = varName.substr(nameStart + 1);
                parameters_[varName] = ShaderParameter{
                    desc_.type_, varName, variable.offset, variable.size, (unsigned)cBufferLookupValue->second};
            }
        }
