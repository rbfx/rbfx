...
public:
    ...
    virtual bool IsMacro() override { return true; }
    virtual bool BeginChild() override { return true; }
    bool enabled_{false};
};
        ...
        while (loop < MAX_SEARCH_LOOP)
        {
            ...
            if (nextToken->IsOperator())
            {
                throw std::runtime_error(
                    Format("Expected a identifier. ({}, {})", nextToken->desc_.col_, nextToken->desc_.line_).c_str());
            }
    ...
    virtual void Process(unsigned& parentSeek, TokenProcessDesc& processDesc) override
    {
        ...
        // Evaluate macro values and get all operators
        {
            ...
            while (loop < children_.size())
            {
                ...
                {
                    throw std::runtime_error(
                        Format("Expected evaluator. ({:d}, {:d})", currToken->desc_.line_, currToken->desc_.col_)
                            .c_str());
                }
                ...
                else
                {
                    ...
                    if (value.starts_with("defined("))
                    {
                        if (!value.ends_with(")"))
                        {
                            throw std::runtime_error(Format("Expected a ')'. ({:d}, {:d})", currToken->desc_.line_,
        ...
        {
            throw std::runtime_error(Format("Expected an evaluator. ({}, {})", desc_.line_, desc_.col_).c_str());
        }

        if (tokenValues.size() - 1 != opTokens.size())
        {
            throw std::runtime_error(Format("Invalid Syntax at ({}, {})", desc_.line_, desc_.col_).c_str());
        }
...
public:
    ...
    virtual bool EndChild() override { return true; }
};
...
public:
    ...
    virtual bool Evaluate(size_t first, size_t second) override { return first > second; }
};
...
static unsigned sTokenHashes[] = {StringHash("#ifdef").ToHash(), StringHash("#if").ToHash(),
    StringHash("#else").ToHash(), StringHash("#elif").ToHash(), StringHash("#endif").ToHash(),
    StringHash("#line").ToHash()};
static TokenCtor sTokenCtors[] = {[]() -> ea::shared_ptr<TokenBase> { return ea::make_shared<IfDefToken>(); },
...
static unsigned sTokenOpHashes[] = {StringHash(">").ToHash(), StringHash("<").ToHash(), StringHash(">=").ToHash(),
    StringHash("<=").ToHash(), StringHash("==").ToHash(), StringHash("!=").ToHash(), StringHash("&&").ToHash(),
    StringHash("||").ToHash()};
