...
#ifdef URHO3D_DEBUG
            data_[i]->cbuffer_->SetDebugName(ConstantBufferDebugNames[i]);
#endif
        }
...
void ConstantBufferManager::PrintDebugOutput()
{
    ...
    for (uint8_t i = 0; i < MAX_SHADER_PARAMETER_GROUPS; ++i)
    {
        ...
        output += Format("#{} nextTicket: {:d} | cbufferSize: {:d} | lastOffset: {:d} | prevTicketDispatched: {:d}\n",
            shaderParameterGroupNames[i], ticketMgr->nextTicket_, ticketMgr->cbufferSize_, ticketMgr->lastOffset_,
        ...
        for (auto ticket = ticketMgr->tickets_.begin(); ticket != ticketMgr->tickets_.end(); ++ticket)
        {
            output += Format("-- [{:d}] size: {:d} | offset: {:d} | corrected offset: {:d}\n", ticket->get()->GetId(),
                ticket->get()->GetSize(), ticket->get()->GetOffset(), offset);
    ...
    for (uint8_t i = 0; i < MAX_SHADER_PARAMETER_GROUPS; ++i)
    {
        ...
        output += Format("#{} size: {:d}\n", ea::string(shaderParameterGroupNames[i]), bufferPair.first);
    }
...
void ConstantBufferManager::CollectBuffer(ShaderParameterGroup grp, size_t newSize)
{
    ...
    URHO3D_LOGDEBUG("Buffer {} has resized down to {:d}=>{:d} | ratio: {:f}", shaderParameterGroupNames[grp],
        bufferPair.first, newSize, ((double)newSize / (double)bufferPair.first));
#endif
}
} // namespace Urho3D
