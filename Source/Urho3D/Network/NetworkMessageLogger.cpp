...
void NetworkMessageLogger::Attach(NetworkConnection* connection)
{
    ...
    connection->OnSendMessage.SubscribeWithSender(connection,
        [this](NetworkConnection* sender, NetworkMessageId messageId, ConstByteSpan data, PacketTypeFlags type, ea::string_view debugInfo, bool& handled)
    {
        if (handled)
            return;

        LogSentMessage(messageId, data, type, debugInfo);
    });

    connection->OnMessageReceived.SubscribeWithSender(connection,
        [this](NetworkConnection* sender, NetworkMessageId messageId, ConstByteSpan data, bool& handled)
    {
        (void)sender;
        (void)handled;
...
void NetworkMessageLogger::LogSentMessage(
    NetworkMessageId messageId, ConstByteSpan data, PacketTypeFlags packetType, ea::string_view debugInfo)
{
    ...
    Log::GetLogger().Write(logLevel, "Message #{} ({} bytes) sent{}{}{}{}", static_cast<unsigned>(messageId),
        data.size(), (packetType & PacketType::Reliable) ? ", reliable" : "",
        (packetType & PacketType::Ordered) ? ", ordered" : "", debugInfo.empty() ? "" : ": ", debugInfo);
}

void NetworkMessageLogger::LogReceivedMessage(NetworkMessageId messageId, ConstByteSpan data, ea::string_view debugInfo)
{
    const LogLevel logLevel = GetMessageLogLevel(messageId);
    if (logLevel == LOG_NONE)
        return;
    ...
    Log::GetLogger().Write(logLevel, "Message #{} ({} bytes) received{}{}", static_cast<unsigned>(messageId),
        data.size(), debugInfo.empty() ? "" : ": ", debugInfo);
}
