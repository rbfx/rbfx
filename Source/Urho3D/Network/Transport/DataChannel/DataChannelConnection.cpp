...
DataChannelConnection::~DataChannelConnection()
{
    ...
    {
        URHO3D_LOGWARNING(
            "DataChannelConnection was not properly disconnected before destruction. Forcing disconnect.");
    }
}
...
{
    if (state_ == NetworkConnectionState::Disconnected || state_ == NetworkConnectionState::Disconnecting)
        return;

    BaseClassName::Disconnect();
#if URHO3D_PLATFORM_WEB
    bool wasConnected = state_ == NetworkConnectionState::Connected;
#endif
...
bool DataChannelConnection::SendData(ConstByteSpan data, PacketTypeFlags type)
{
    ...
    {
        URHO3D_LOGERROR(
            "DataChannel tried to send {} bytes of data, which is more than max allowed {} bytes of data per message.",
    ...
    if (auto* dc = dataChannels_[type].get())
    {
        if (dc->isOpen())
        {
            ...
            dc->send(reinterpret_cast<const rtc::byte*>(data.data()), dataSize);
            return true;
        }
...
{
    if (state_ != NetworkConnectionState::Connected || !dataChannels_[0])
        return 0;
...
void DataChannelConnection::SetPortRange(unsigned begin, unsigned end)
{
    portRangeBegin_ = begin;
    portRangeEnd_ = end;
}

void DataChannelConnection::InitializeWithWebSocket(std::shared_ptr<rtc::WebSocket> ws, DataChannelServer* server)
{
    InitializeFromSocket(server, ws);
    websocketWasOpened_ = true;
}
...
void DataChannelConnection::OnDataChannelDisconnected(int index, bool notifyCallbacks)
{
    ...
    for (int i = 0; i < URHO3D_ARRAYSIZE(dataChannels_); i++)
    {
        if (dataChannels_[i] && state_ != NetworkConnectionState::Connecting)
            return;
    }
...
void DataChannelConnection::InitializeFromSocket(DataChannelServer* server, std::shared_ptr<rtc::WebSocket> websocket)
{
    ...
    if (server != nullptr)
    {
        ...
        dataChannels_[PacketType::UnreliableUnordered] =
            peer_->createDataChannel("uu", {{rtc::Reliability::Type::Rexmit, true, 0}});
        dataChannels_[PacketType::Reliable] =
            peer_->createDataChannel("ru", {{rtc::Reliability::Type::Reliable, true}});
        dataChannels_[PacketType::Ordered] =
            peer_->createDataChannel("uo", {{rtc::Reliability::Type::Rexmit, false, 0}});
        dataChannels_[PacketType::ReliableOrdered] =
            peer_->createDataChannel("ro", {{rtc::Reliability::Type::Reliable, false}});
        for (int i = 0; i < URHO3D_ARRAYSIZE(dataChannels_); i++)
        {
            ...
            {
                const ConstByteSpan message(reinterpret_cast<const unsigned char*>(data.data()),
                    static_cast<ConstByteSpan::index_type>(data.size()));
                DispatchDataReceived(message);
            }, [](rtc::string) {});
        }
    }
